...
export default function CmsPlayerPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/cms/player/settings`)
  ...
  const fetchAndSync = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/cms/player/${slug}/active-content`);
  ...
  const reportHealth = useCallback(() => {
    getStorageUsageBytes().then(storageUsageBytes => {
      ...
      fetch(`${API_BASE}/cms/player/${slug}/health`, {
        method: 'POST',
  ...
  const pollCommands = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/cms/player/${slug}/commands`);
      if (!res.ok) return;
      ...
      for (const command of commands) {
        ...
        switch (command.commandType) {
          case 'REFRESH':
            await fetch(`${API_BASE}/cms/player/commands/${command.id}/ack`, { method: 'POST' }).catch(() => undefined);
