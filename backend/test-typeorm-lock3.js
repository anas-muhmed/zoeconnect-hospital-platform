...
async function run() {
    ...
    try {
        const c1 = new TestLock();
        c1.id = e.id;
        c1.name = 'V1';
        c1.version = e.version;
        await repo.save(c1);
        console.log('Saved C1');
        const c2 = new TestLock();
        c2.id = e.id;
        c2.name = 'V2';
        c2.version = e.version;
        await repo.save(c2);
        console.log('Saved C2 (SHOULD NOT HAPPEN)');
    }
    catch (err) {
...
run();
//# sourceMappingURL=test-typeorm-lock3.js.map